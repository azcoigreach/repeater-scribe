...
function renderMention(mention) {
  ...
  const link = text('a', 'Open recording', 'control-button'); link.href = mention.recording_url; card.append(link);
  if (mention.audio_available && mention.start_offset != null) { const seek = text('button', 'Play from mention', 'control-button'); seek.type = 'button'; seek.addEventListener('click', () => playMention(seek, mention)); card.append(seek); } else card.append(text('p', `Audio ${mention.audio_status || 'unavailable'}`, 'muted-text'));
...
async function loadProfile() {
  ...
  if (item.qrz_image_url) { const image = document.createElement('img'); image.className = 'callsign-profile-image'; image.src = item.qrz_image_url; image.alt = `QRZ profile image for ${item.callsign}`; image.referrerPolicy = 'no-referrer'; profile.append(image); }
  if (item.qrz_profile_url) { const qrz = text('a', 'View QRZ profile', 'callsign-evidence'); qrz.href = item.qrz_profile_url; qrz.target = '_blank'; qrz.rel = 'noopener noreferrer'; profile.append(qrz); }
