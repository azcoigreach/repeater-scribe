(() => {
  ...
  const restore = () => { const query = new URLSearchParams(location.search); ['q', 'status', 'audio_status', 'callsign'].forEach(name => { if (query.has(name)) form.elements[name].value = query.get(name); }); ['from', 'to'].forEach(name => { if (query.has(name)) form.elements[name].value = query.get(name).slice(0, 10); }); };
  ...
  const renderChips = () => { chips.replaceChildren(); fields.filter(field => field.value).forEach(field => { const chip = document.createElement('span'); chip.className = 'filter-chip'; chip.append(document.createTextNode(`${field.labels[0].textContent}: ${field.value} `)); const remove = document.createElement('button'); remove.type = 'button'; remove.textContent = 'Clear'; remove.setAttribute('aria-label', `Clear ${field.labels[0].textContent} filter`); remove.addEventListener('click', () => { field.value = ''; resetAndLoad(true); }); chip.append(remove); chips.append(chip); }); };
  ...
  async function load(append = false) { if (loading || !validRange()) return; loading = true; more.disabled = true; const version = ++requestVersion; if (controller) controller.abort(); controller = new AbortController(); const query = params(); query.set('limit', '50'); if (append && cursor) query.set('cursor', cursor); if (!append) { rows.replaceChildren(); cards.replaceChildren(); loaded = new Set(); cursor = null; setState('Loading archive...'); } try { const response = await fetch(`/api/v1/archive/recordings?${query}`, { signal: controller.signal, headers: { Accept: 'application/json' } }); if (version !== requestVersion) return; if (!response.ok) { if (response.status === 401 || response.status === 403) throw new Error('Your session cannot access the archive. Sign in again.'); if (response.status === 422 && append) throw new Error('The archive cursor is no longer valid. Restart the search.'); throw new Error('Archive request failed.'); } const payload = await response.json(); payload.items.forEach(render); cursor = payload.next_cursor; more.hidden = !payload.has_more; count.textContent = `${loaded.size} record${loaded.size === 1 ? '' : 's'} loaded`; setState(!loaded.size ? (query.size ? 'No recordings match these filters.' : 'No recordings are in the catalog yet.') : ''); } catch (error) { if (error.name !== 'AbortError') { setState(error.message, true); } } finally { if (version === requestVersion) { loading = false; more.disabled = false; } } }
  ...
  window.addEventListener('popstate', () => { restore(); renderChips(); load(false); });
