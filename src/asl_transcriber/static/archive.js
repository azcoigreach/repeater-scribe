(() => {
  ...
  const params = () => { const result = new URLSearchParams(); ['q', 'status', 'audio_status', 'callsign', 'source_id', 'tag'].forEach(name => { if (value(name)) result.set(name, value(name)); }); const from = value('from'); const to = value('to'); if (from) result.set('from', new Date(from.includes("T") ? from : `${from}T00:00:00`).toISOString()); if (to) result.set('to', new Date(to.includes("T") ? to : `${to}T23:59:59.999`).toISOString()); return result; };
  const restore = () => { const query = new URLSearchParams(location.search); ['q', 'status', 'audio_status', 'callsign', 'source_id', 'tag'].forEach(name => { form.elements[name].value = query.get(name) ?? ''; }); ['from', 'to'].forEach(name => { const stored = query.get(name); form.elements[name].value = stored ? stored.slice(0, 16) : ''; }); };
  ...
  document.querySelector('#event-from-selection').addEventListener('click', () => {
    ...
    const starts = values.filter(item => item.started_at).map(item => new Date(item.started_at).getTime());
    const ends = values.filter(item => item.started_at).map(item => new Date(item.started_at).getTime() + Math.max(item.duration_seconds || 0, 0.001) * 1000);
