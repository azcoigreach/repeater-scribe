(() => {
  ...
  const formatTime = iso => UITime.format(iso, 'Unknown time');
  ...
  const callNodes = item => { const fragment = document.createDocumentFragment(); callsigns(item).forEach(call => { const wrapper = document.createElement('span'); wrapper.className = 'callsign-nav'; const button = document.createElement('button'); button.type = 'button'; button.className = 'callsign'; button.textContent = call; button.title = 'Filter archive by callsign'; button.addEventListener('click', () => { form.elements.callsign.value = call; resetAndLoad(true); }); const link = document.createElement('a'); link.href = `/callsigns/${encodeURIComponent(call)}`; link.className = 'callsign-evidence'; link.textContent = 'View station history'; link.title = `View ${call} station history`; wrapper.append(button, link); fragment.append(wrapper); }); return fragment; };
  const params = () => { const result = new URLSearchParams(); ['q', 'status', 'audio_status', 'callsign', 'source_id', 'tag'].forEach(name => { if (value(name)) result.set(name, value(name)); }); const from = value('from'); const to = value('to'); if (from) result.set('from', UITime.inputUTC(form.elements.from)); if (to) result.set('to', UITime.inputUTC(form.elements.to, true)); return result; };
  const restore = () => { const query = new URLSearchParams(location.search); ['q', 'status', 'audio_status', 'callsign', 'source_id', 'tag'].forEach(name => { form.elements[name].value = query.get(name) ?? ''; }); ['from', 'to'].forEach(name => { const stored = query.get(name); UITime.setInput(form.elements[name], stored, name === 'to'); }); };
  const updateUrl = push => { const query = params(); const url = `${location.pathname}${query.size ? `?${query}` : ''}`; history[push ? 'pushState' : 'replaceState']({}, '', url); };
  ...
  const validRange = () => { let from, to; try { from = UITime.inputUTC(form.elements.from); to = UITime.inputUTC(form.elements.to, true); } catch (error) { filterError.hidden = false; filterError.textContent = error.message; return false; } if (from && to && UITime.instant(from) > UITime.instant(to)) { filterError.hidden = false; filterError.textContent = 'The start date must not be after the end date.'; return false; } filterError.hidden = true; return true; };
  ...
  const render = item => { if (loaded.has(item.id)) return; loaded.add(item.id); const row = document.createElement('tr'); const time = document.createElement('time'); time.dateTime = item.started_at || ''; time.title = formatTime(item.started_at); addText(time, formatTime(item.started_at)); const cells = [time, formatDuration(item.duration_seconds), callNodes(item), item.local_node || item.source_node || '—', tag(item.status, item.status), tag(item.audio_status, item.audio_status), item.transcript?.display_text || item.transcript?.raw_text || 'No transcript']; cells.forEach((content, index) => { const cell = document.createElement('td'); if (index === 6) { const excerpt = document.createElement('p'); excerpt.className = 'excerpt'; addText(excerpt, content); cell.append(excerpt); } else if (content instanceof Node) cell.append(content); else addText(cell, content); row.append(cell); }); const action = document.createElement('td'); const link = document.createElement('a'); link.className = 'detail-link'; link.href = recordingLink(item.id); link.textContent = 'Open details'; action.append(link); addSelection(action, item); row.append(action); rows.append(row);
    const card = document.createElement('article'); card.className = 'recording-card'; const header = document.createElement('header'); header.append(time.cloneNode(true), tag(item.audio_status, item.audio_status)); card.append(header); const meta = document.createElement('p'); meta.className = 'meta'; meta.textContent = `${formatDuration(item.duration_seconds)} · ${item.status}`; card.append(meta); const text = document.createElement('p'); text.className = 'excerpt'; addText(text, item.transcript?.display_text || item.transcript?.raw_text || 'No transcript'); card.append(text); const callLine = document.createElement('p'); callLine.append(callNodes(item)); card.append(callLine); const cardLink = link.cloneNode(true); card.append(cardLink); addSelection(card, item); cards.append(card);
  };
  ...
  ['from', 'to', 'status', 'audio_status', 'source_id', 'tag'].forEach(name => form.elements[name].addEventListener('change', () => resetAndLoad(true)));
  ...
  function addSelection(parent, item) {
    ...
    checkbox.addEventListener('change', () => {
      ...
      document.querySelectorAll('[data-select-recording]').forEach(input => input.checked = selected.has(input.dataset.selectRecording));
      document.querySelector('#event-from-selection').textContent = `Create event from selected (${selected.size})`;
    });
    label.append(checkbox, document.createTextNode('Select for event'));parent.append(label);
  }
  document.querySelector('#event-from-range').addEventListener('click', () => {
    if (!value('source_id') || !value('from') || !value('to') || !validRange()) {setState('Choose a monitored source and both times to create a historical event.', true);return;}
    const range = params();const query = new URLSearchParams({create:'historical', source_id:value('source_id'), from:range.get('from'), to:range.get('to')});
    location.assign(`/events?${query}`);
  });
  document.querySelector('#event-from-selection').addEventListener('click', () => {
    const values = [...selected.values()];
    if (!values.length || new Set(values.map(item => item.source_id)).size !== 1) {setState('Select recordings from exactly one monitored source.', true);return;}
    const starts = values.filter(item => item.started_at).map(item => UITime.instant(item.started_at).getTime());
    const ends = values.filter(item => item.started_at).map(item => UITime.instant(item.started_at).getTime() + Math.max(item.duration_seconds || 0, 0.001) * 1000);
    const query = new URLSearchParams({create:'historical', selection:'1', source_id:values[0].source_id});
    if (starts.length) {query.set('from', new Date(Math.min(...starts)).toISOString());query.set('to', new Date(Math.max(...ends)).toISOString());}
    sessionStorage.setItem('event-selection', JSON.stringify(values.map(item => item.id)));
    location.assign(`/events?${query}`);
  });
  fetch('/api/v1/sessions/sources').then(response => {if (!response.ok) throw new Error('Sources unavailable');return response.json();}).then(payload => {
    payload.items.forEach(item => form.elements.source_id.add(new Option(item.label, item.id)));
